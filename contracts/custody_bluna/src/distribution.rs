...
use cosmwasm_std::{
    attr, to_binary, Addr, Coin, CosmosMsg, Deps, DepsMut, Env, MessageInfo,
...
use crate::state::{
    read_config, read_global_index, save_global_index, BLunaAccruedRewardsResponse, Config,
};
...
pub fn distribute_hook(
    ...
    let new_global_index =
        read_global_index(deps.storage) + Decimal256::from_ratio(reward_amount, collateral_amount);
    save_global_index(deps.storage, &new_global_index)?;
